...
$( '.download-btn' ).click( function() {
	...
	if ( $current_product in $products ) {
		...
		top.location.replace( $products[ $current_product ][ 'download' ] );
...
$( '.desktop-btn' ).on( 'click', function() {
    ...
    document.getElementById('product-iframe').contentWindow.location.reload(true);
...
$( document ).ready( function() {
	...
	if ( ! ( $current_product in $products ) || $current_product === '' ) {
        ...
        if ($current_product === '')
            $( '.product-switcher a' ).trigger('click');
